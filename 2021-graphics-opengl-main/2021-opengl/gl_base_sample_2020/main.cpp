...
int main(int argc, char** argv)
{
    ...
    ////////////////////////////
    GLsizei sphereIndices = CreateSphere(1.0f, 50.0f, vaoSphere);
    ...
    GLsizei Cylinder = CreateCylinder(1.0f, 5.0f, 120, vaoCylinder);
    GLsizei Stairs = CreateCuboid(1.0f, 1.0f, 1.0f, vaoStairs);
    GLsizei Wheel = CreateSphere(1.0f, 250, vaoWheel);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            if (timer < 101) {
                model = transpose(mul(rotate_Y_4x4(timer * 10 * LiteMath::DEG_TO_RAD), (mul(translate4x4(float3(3.0f, timer / 2, 3.0f)), scale4x4(float3(1.0f, 1.0f, 1.0f))))));
        ...
        {
            float4x4 model = transpose(mul(translate4x4(float3(-20.0f, 0.0f, -20.0f)), scale4x4(float3(10.0f, 10.0f, 10.0f))));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
        ...
        {
            model = transpose(translate4x4(float3(0.0f, 0.0f, 0.0f)));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
        ...
        {
            model = transpose(mul(translate4x4(float3(-17.5f, 5.0f, -17.5f)), scale4x4(float3(2.0f, 1.0f, 2.0f))));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
        ...
        for (unsigned int i = 1; i < 4; i++) {
            ...
            {
                model = transpose(mul(rotate_Y_4x4(timer * 10 * LiteMath::DEG_TO_RAD), mul(rotate_X_4x4(90 * LiteMath::DEG_TO_RAD), mul(translate4x4(float3(10.0f, 10.0f, 0.5*i-1.0f-2)), scale4x4(float3(i*0.2f, 1.0f, 0.3f))))));
                lambert.SetUniform("model", model); GL_CHECK_ERRORS;
        ...
        /// Лестница
        glBindVertexArray(vaoStairs);
        {
            float4x4 model = transpose(mul(translate4x4(float3(-0.5f, 0.0f, -0.5f)), scale4x4(float3(1.0f, 50.0f, 1.0f))));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Stairs, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            for (int i = 0; i < 100; i++)
            {
                float4x4 model = transpose(mul(rotate_Y_4x4(i * 10 * LiteMath::DEG_TO_RAD), (mul(translate4x4(float3(0.0f, 0.5 * i + 0.0f, 0.0f)), scale4x4(float3(4.0f, 0.5f, 1.0f))))));
                lambert.SetUniform("model", model); GL_CHECK_ERRORS;
                glDrawElements(GL_TRIANGLE_STRIP, Stairs, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            }
        }
        ...
        {
            float4x4 model = transpose(mul(rotate_Y_4x4(timer * 10 * LiteMath::DEG_TO_RAD), mul(translate4x4(float3(10.0f, 1.0f, 10.0f)), scale4x4(float3(0.3f, 1.0f, 1.0f)))));
            lambert.SetUniform("model", model); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Wheel, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            {
                float4x4 model = transpose(mul(rotate_Y_4x4(timer * 10 * LiteMath::DEG_TO_RAD), (mul(translate4x4(float3(10.0f, 1.0f, 15.0f)), scale4x4(float3(0.3f, 1.0f, 1.0f))))));
                lambert.SetUniform("model", model); GL_CHECK_ERRORS;
                glDrawElements(GL_TRIANGLE_STRIP, Wheel, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            }
